...
leg.SetFillStyle(0)
leg.SetBorderSize(0)
#lname = ["Phlegyas Run 296786","Kerberos Run 296786 PU40","Kerberos Run 297050 PU33"]
...
while k< len(Thists):
    ...
    elif not files[k].find("Menu-")==-1:
        menu=''
        menu='Menu-'+(files[k].split('Menu-')[1]).split('_')[0]
    else:
        menu=''
    #now do release
    ...
    if k==0:
        ...
        Thists[k].GetYaxis().SetTitleOffset(1)
...
#        name += type+' '+pu+' '+menu+' '+release+" Mean: %3.2f ms (%s)"% (Thists[k].GetMean(),lname[k])
        #if args.ext:
...
if args.outfile:
    ...
    filename2 = 'HLT_Validation_Plot.png'
c1.Print(filename)
c1.Print(filename2)
